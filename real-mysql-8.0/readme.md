...
- 트랜잭션  
작업의 완전성을 보장한다. 논리적인 작업 셋을 모두 완벽하게 처리하거나, 처리 못할 경우에는 기존 상태로 복구해서 작업의 일부(Partial update)만 적용되지 않게 만들어주는 기능이다.
- 잠금  
동시성을 제어하기 위한 기능이다.
...
- 레코드 락  
레코드 자체만을 잠근다. 프라머리 키 또는 유니크 인덱스 조건을 이용한 변경에는 레코드 락을 사용한다.
- 갭 락  
