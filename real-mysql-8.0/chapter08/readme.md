...
클러스터링 테이블의 구조 자체는 일반 B-Tree와 비슷하지만, 클러스터링 인덱스의 리프 노드에는 레코드의 모든 칼럼이 같이 저장돼 있다.
...
유니크 엔덱스에는 NULL도 저장될 수 이쓴데, NULL은 특정 값이 아니므로 2개 이상 저장될 수 있다.
...
외래키 제약이 설정되면 자동으로 연관된 테이블 커럶에 인덱스까지 생성된다.
