...
분할 정복을 적용해 문제를 해결하기 위해서는 문제에 두 가지 특성이 성립해야 한다.
문제를 둘 이상의 부분 문제로 나누는 자연스러운 방법이 있어야 하며, 부분 문제의 답을 계산하는 효율적인 방법이 있어야 한다.
...
Combinatorial game이란 체스나 오목처럼 두 사람의 참가자가 하는 게임을 가르킨다.
...
부분 문제 간의 의존성을 파악하기 쉬울 경우에는 반복문을 이용해서 동적 계획법을 구현할 수 있다.

슬라이딩 윈도우(sliding window)란 사용하는 데이터 전체를 메모리에 유지하는 것이 아니라 필요한 부분만을 저장하는 기법을 가리키는 말이다.

## Greedy method

원하는 답을 여러 개의 조각으로 쪼개고, 각 단계마다 답의 한 부분을 만들어 간다는 점에서 완전 탐색이나 동적 계획법 알고리즘과 다를 것이 없다.
그러나 탐욕법은 각 단계마다 지금 당장 가장 좋은 방법만을 선택한다.

### 탐욕적 알고리즘이 사용하는 두 가지 경우

1. 탐욕법을 사용해도 항상 최적해를 구할 수 있는 문제를 만난 경우
2. 시간이나 공간적 제약으로 인해 최적해를 찾기 너무 어렵다면 최적해 대신 근사해를 찾는 것으로 타협하는 경우

### 정당성 증명

- greedy choice property  
답의 모든 부분을 고려하지 않고 탐욕적으로만 선택하더라도 최적해를 구할 수 있다.
다른 최적해가 존재함을 가정하고, 이것을 조작해서 우리가 선택한 답으로 바꿀 수 있음을 보인다.
- optimal substructure  
